...
*/
      ...
      // move it to a more-sensible location:
      this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(2.7,-1.0, 3.0);  // move rightwards (+x),
...
CScene.prototype.traceRay = function(inRay, myHit) {
//==============================================================================
// Test all CGeom objections in CScene for intersection
// Modiefies myHit to describe hit point nearest to camera
  for(k=0; k< this.item.length; k++) {  // for every CGeom in item[] array,
    this.item[k].traceMe(inRay, myHit);  // trace eyeRay thru it,
  } 
...
//==============================================================================
// Call CGeom of myHit
  // Get material constant of hit geom, and find view, normal, light, reflection vectors
// Create shadow ray towards light to call traceRay, if hit anything, lower difffuse/reflective...
// Find color of reflected ray
  // Create reflection ray with surface normal and view ray
...
//	console.log("You called CScene.makeRayTracedImage!")
  ...
  // Init array of rays for samples of each pixel MASON
  for (i=0; i < g_AAcode * g_AAcode; i++) {
  ...
  for(j=0; j< this.imgBuf.ySiz; j++) {        // for the j-th row of pixels.
      ...
      // Create ray for each sample MASON
      var u,v;
      var sample_w = 1/g_AAcode; // width of each sample area
      vec4.set(colr, 0, 0, 0, 0);   // reset color of pixel
      for (u = 0; u < g_AAcode; u++) {
        for (v = 0; v < g_AAcode; v++) {
          // find overall ray pos in image and make ray
          if (g_isJitter) {
            sample_i = i + u * sample_w + Math.random() * sample_w;
            sample_j = j + v * sample_w + Math.random() * sample_w;
          }
          else {
            sample_i = i + u * sample_w + 0.5 * sample_w;
            sample_j = j + v * sample_w + 0.5 * sample_w;
          }
          ...
          // Refactor below findShade into function MASON
          this.findShade(myHit)

          // add color of myHit ray to pixel color sum
          vec4.add(colr, colr, myHit.colr);
        }
      }
			//this.rayCam.setEyeRay(this.eyeRay,i+0.5,j+0.5);  // create ray for pixel (i,j)

      // average color of pixel
      var num_samples = g_AAcode * g_AAcode;
      vec4.divide(colr, colr, [num_samples, num_samples, num_samples, num_samples]);
      ...
      myHit.init();     // start by clearing our 'nearest hit-point', and
      for(k=0; k< this.item.length; k++) {  // for every CGeom in item[] array,
          this.item[k].traceMe(this.eyeRays[0], myHit);  // trace eyeRay thru it,
...
*/  
      ...
      /*// Refactor below findShape into function MASON
      // Get material constant of hit geom, and find view, normal, light, reflection vectors
      // Create shadow ray towards light to call traceRay, if hit anything, 0.3 * difffuse/reflective...
      // Create reflection ray with surface normal and view ray
      // Call traceRay to find CHit
      // Call findshade to get color at point and use for phong shading
